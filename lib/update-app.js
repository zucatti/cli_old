module.exports = function (cbz) {
    ...
    function update_settings(cb) {
        fs.writeFile(global.dir + '/config/settings.json', JSON.stringify(global.settings, null, 4), cb);
    }
    function update_package_json(ff, cb) {
        fs.writeFile(global.project.bin + '/package.json', JSON.stringify(ff, null, 4), cb);
    };
    ...
    function updatePackages(cb) {
        ...
        function do_pkg(p, ndx, installed, cbo) {
            ...
            for (var z = 0; z < installed.length; z++) {
                if (installed[z].split(':').length > 1) {
                    _installed.push(installed[z].split(':')[0] + ':' + installed[z].split(':')[1].substr(1, installed[z].length));
                }
            };
            if (installed.indexOf(item) > -1) return do_pkg(p, ndx + 1, installed, cbo);
            if (_installed.indexOf(item) > -1) return do_pkg(p, ndx + 1, installed, cbo);
                ...
                task: () => {
                    return new Observable(observer => {
                        ...
                        child.on('close', function () {
                            observer.complete();
                            fs.readFile(global.project.bin + '/node_modules/' + item.split(':')[0] + '/package.json', 'utf-8', function (e, r) {
        ...
        fs.readFile(global.project.bin + '/package.json', 'utf-8', function (e, r) {
            if (e) var package_json = {
            ...
            var pkg_installed = [];
            for (var el in package_json.dependencies) pkg_installed.push(el + ':' + package_json.dependencies[el]);
            var _installed = [];
            for (var z = 0; z < pkg_installed.length; z++) {
                if (pkg_installed[z].split(':').length > 1) {
                    _installed.push(pkg_installed[z].split(':')[0] + ':' + pkg_installed[z].split(':')[1].substr(1, pkg_installed[z].length));
                }
            };
                ...
                fs.writeFile(global.dir + '/manifest.yaml', yaml.stringify(global.manifest), function () {
                    ...
                    while (i--) {
                        if (pkg.indexOf(_installed[i]) == -1) {
                            ...
                            delete package_json.dependencies[_installed[i].split(':')[0]];
                        }
                    };
                    if (uninstall.length > 0) console.log(chalk.bold('- Cleaning packages '));
                    if (uninstall.length > 0) return update_package_json(package_json, function () {
                        shelljs.cd(global.project.bin);
                        shelljs.exec('npm install', { silent: true }, function () {
    ...
    function updateAuth(cb) {
        function do_auth(a, ndx, cbo) {
            ...
            for (var i = 0; i < global.settings.auth.length; i++) {
                if (global.settings.auth[i].name == a[ndx]) return do_auth(a, ndx + 1, cbo);
            };
